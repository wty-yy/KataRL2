...
if __name__ == '__main__':
    ...
    rms.update(np.array([1.0, 2.0, 4.0]))
    print(rms.get_statistics())

    # vector
    rms = RunningMeanStd(shape=(2,3,4))
    rms.update(np.random.randn(2,3,4))
    rms.update(np.random.randn(10,2,3,4))
    print(rms.get_statistics())

    import torch
    torch.save(rms, "rms.pkl")
    del rms

    rms = torch.load("rms.pkl", weights_only=False)
    print(rms.get_statistics())
