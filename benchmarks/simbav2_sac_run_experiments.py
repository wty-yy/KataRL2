...
tasks = [
    ...
    # Mujoco
    ("env:gym", "Ant-v4",                   [0, 0, 0], [0, 1, 2]),
    ("env:gym", "HalfCheetah-v4",           [0, 0, 0], [0, 1, 2]),
    ("env:gym", "Hopper-v4",                [1, 1, 1], [0, 1, 2]),
    ("env:gym", "HumanoidStandup-v4",       [3, 3, 3], [0, 1, 2]),
    ("env:gym", "Humanoid-v4",              [1, 1, 1], [0, 1, 2]),
    ...
    ("env:dmc", "walker-stand",             [2, 2, 2], [0, 1, 2]),
    ("env:dmc", "walker-walk",              [3, 3, 3], [0, 1, 2]),
    ("env:dmc", "walker-run",               [3, 3, 3], [0, 1, 2]),
    # DMC HARD
    ("env:dmc", "humanoid-walk",            [4, 4, 4], [0, 1, 2]),
    ("env:dmc", "dog-walk",                 [4, 4, 4], [0, 1, 2]),
    ("env:dmc", "humanoid-run",             [5, 5, 5], [0, 1, 2]),
    ("env:dmc", "dog-run",                  [5, 5, 5], [0, 1, 2]),
...
if __name__ == "__main__":
    ...
    for env_subcommand, env_name, cuda_list, seed_list in tasks:
        for cuda_id, seed in zip(cuda_list, seed_list):
            ...
            log_file = str(PATH_NOHUP_OUT_DIR / f"simbav2_sac_{env_subcommand}_{env_name}_seed{seed}_{time.strftime('%Y%m%d_%H%M%S')}.log")
